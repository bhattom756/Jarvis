...
const BACKEND_URL = import.meta.env.VITE_JARVIS_BACKEND_URL ?? "http://127.0.0.1:8000";
...
export function EmailComposer() {
  ...
  async function submit() {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/emails/send`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recipients: recipientList, subject, body, attachments: attachmentList })
      });
  ...
  async function refreshUnread() {
    try {
      const response = await fetch(`${BACKEND_URL}/emails/unread`);
